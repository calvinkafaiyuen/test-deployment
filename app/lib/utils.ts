...
export const generateResetPwdLink = (token : string): string => {
  return `localhost:3000/reset-password?token=${token}`;
}

export const sendResetPwdLink = (url : string, email : string) => {
  try{
    fetch(`${process.env.NEXT_PUBLIC_API_URL}send-email`, {
      method: 'POST',
      headers: {
