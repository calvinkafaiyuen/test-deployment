...
import { deleteResourcesHandler, resourcesHandler } from "@/app/lib/actions/admin/resources.action";
...
const getOption = (pathname: string) => {
  ...
  const optionIndex = parts.indexOf('resources');
  return parts[optionIndex + 1] || 'defaultProgram';
}

const fetchResourcesData = async (pathname: string, searchQuery: string = '', status: string = 'all'): Promise<any> => {
  console.log(`pathname: ${pathname}`);
  const option = getOption(pathname);

  try {
      const apps = await resourcesHandler(option, searchQuery, status);
      console.log(`fetchResourcesData:`, apps);
...
export default function Page() {
  ...
  const deleteSelectedRows = () => {
    ...
    deleteResourcesHandler(selectedRowsData, option)
