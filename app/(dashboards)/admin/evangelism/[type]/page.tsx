...
import { deleteEvangelismHandler, evangelismHandler } from "@/app/lib/actions/admin/evangelism.action";
...
const getOption = (pathname: string) => {
  ...
  const optionIndex = parts.indexOf('evangelism');
  return parts[optionIndex + 1] || 'defaultProgram';
}

const fetchEvangelismData = async (pathname: string, searchQuery: string = '', status: string = 'all'): Promise<any> => {
  console.log(`pathname: ${pathname}`);
  const option = getOption(pathname);

  try {
      const apps = await evangelismHandler(option, searchQuery, status);
      console.log(`fetchEvangelismData:`, apps);
...
export default function Page() {
  ...
  const deleteSelectedRows = () => {
    ...
    deleteEvangelismHandler(selectedRowsData, option)
