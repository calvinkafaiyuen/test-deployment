...
import { configurationsHandler, deleteConfigurationHandler } from "@/app/lib/actions/admin/configuration.action";
...
const getOption = (pathname: string) => {
  ...
  const optionIndex = parts.indexOf('configurations');
  return parts[optionIndex + 1] || 'defaultProgram';
}

const fetchConfigurationData = async (pathname: string, searchQuery: string = '', status: string = 'all'): Promise<any> => {
  console.log(`pathname: ${pathname}`);
  const option = getOption(pathname);

  try {
      const apps = await configurationsHandler(option, searchQuery, status);
      console.log(`fetchConfigurationData:`, apps);
...
export default function Page() {
  ...
  useEffect(() => {
    fetchConfigurationData(pathname)
      .then((data: any) => {
  ...
  const deleteSelectedRows = () => {
    ...
    deleteConfigurationHandler(selectedRowsData, option)
  ...
  return (
    <div className="ag-theme-alpine grid-container">
      <div id="top-subnavigation" className="flex align-middle mb-4 text-lg text-[#4b5563] items-center">
        <TopSubNavigation
          fetchApps={fetchConfigurationData}
